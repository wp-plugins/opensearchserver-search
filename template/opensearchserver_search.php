...
if (isset($result) && $result instanceof SimpleXMLElement) {
	...
	if($ossResults->getResultFound()<=0) {?>
		...
		Your Search Keyword <b><?php print "'	".$query. "	'";?></b> did not match any document<br/>Suggestions:<br/>
  ...
  <div id="oss_filter">
  <ul id="oss_ul">
  <li><a href="<?php print '?s='.urlencode($query).'&fq=All';?>">All</a></li>
  <?php foreach ($ossResults->getFacet('type') as $values) {
    $value = $values['name'];
    ?>
    <li><a id="oss_link" href="?s=<?php print $query.'&fq='.$value ?>"><?php print $value.'('.$values.')';?></a></li>
 <?php } ?>
  </ul></div>
  </td><td rowspan="2" style="border:none">
...
$oss_paging=opensearchserver_paging($result);
	foreach($oss_paging as $page)  {
		if($page['url']) {?>
		<span style="margin-left:5px;"><a href="<?php print $page['url'];?>"><?php print $page['label'];?></a></span>&nbsp;
		<?php }
		else { ?>
		<span style="margin-left:5px;"><?php print $page['label'];?></span>&nbsp;
		<?php }
	}?>
	<div align="right">
